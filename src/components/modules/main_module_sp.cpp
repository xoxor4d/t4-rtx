...
namespace components
{
	...
	main_module_sp::main_module_sp()
	{
		...
		utils::hook(0x6D6B40, sp::register_dvars_stub, HOOK_JUMP).install()->quick();
		...
		utils::hook(0x732CF7, sp::force_lod_stub01, HOOK_JUMP).install()->quick();

		// ^ (R_AddAllStaticModelSurfacesRangeSunShadow)
		utils::hook::nop(0x7332A3, 6);
		utils::hook(0x7332A3, sp::force_lod_stub02, HOOK_JUMP).install()->quick();
